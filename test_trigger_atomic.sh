#!/bin/bash
# Regression test for the ReSpeaker 6-mic seeed-voicecard/ac108 driver.
# Covers BOTH shipped fixes:
#   (1) sleep-in-atomic in the PCM .trigger (capture START) -> "scheduling while
#       atomic ... seeed_voice_card_trigger", intermittently fatal.
#   (2) register/unregister UAF on module unbind/shutdown (devm register vs manual
#       unregister+kfree) -> Oops / instruction-abort during teardown.
# The kernel is our assertion engine (might_sleep / slab poisoning / fault handler);
# this harness just drives the paths and counts the signatures.
#
# MODES:
...
*)
  echo "usage: sudo $0 [check|capture|run [cycles]|unbind]"; exit 2
  ;;
esac
